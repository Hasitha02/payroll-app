import React from "react";
import InputField from "../components/PayrollSystem/InputField";
import ActionButtons from "../components/PayrollSystem/ActionButtons";
import EmployeeButton from "../components/PayrollSystem/EmployeeButton";
import { containerStyle, cardStyle, headerStyle, dividerStyle } from "../components/PayrollSystem/styles";

const PayrollSystem = () => {
  return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        <h2 style={headerStyle}>Payroll System</h2>

        {/* EPF and ETF Section */}
        <InputField
          label="EPF Employee Contribution"
          placeholder="Enter employee EPF %"
        />
        <InputField
          label="EPF Company Contribution"
          placeholder="Enter company EPF %"
        />
        <InputField
          label="ETF Rate"
          placeholder="Enter ETF rate %"
        />

        <hr style={dividerStyle} />

        {/* Processing and OT Section */}
        <InputField
          label="Current Processing Year"
          placeholder="YYYY"
        />
        <InputField
          label="Current Processing Month"
          placeholder="MM"
        />
        <InputField
          label="OT Rate"
          placeholder="Enter OT rate per hour"
        />

        {/* Action Buttons */}
        <ActionButtons />

        {/* Employee Navigation Button */}
        <EmployeeButton />
      </div>
    </div>
  );
};

export default PayrollSystem;